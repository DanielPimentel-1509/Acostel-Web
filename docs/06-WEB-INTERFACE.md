...
| Sales / Leads | El botón y los enlaces de WhatsApp viven en el código del sitio (`js/common.js`), pero la gestión del contacto en sí (qué pasa después del clic) es responsabilidad de ese otro módulo, no de este. |
...
- `index.html` — página de inicio: catálogo con pestañas (propiedades/vehículos) y filtros.
...
- Publishing, que despliega cualquier cambio del repositorio automáticamente.
...
- Cualquier almacenamiento de leads — un clic en WhatsApp abre una conversación externa; el sitio no registra que ocurrió.
...
- **Canal estructurado de leads** — un formulario de contacto con datos capturados (nombre, contacto, mensaje, referencia de la ficha), ya definido como "debería tener" en el MVP de `docs/00-VISION.md` — hoy el sitio no sabe si un visitante contactó o no.
